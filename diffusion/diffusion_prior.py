    ...
    def train(self, dataloader,
        ...
        for epoch in range(num_epochs):
            ...
            for x, label, text, text_features, img, img_features, depth_features, aug_img_features, img_index, index \
                ...
                selected_features = (self._select_features(img_features, depth_features, text_features) * 10.0).to(
                    device)
                ##### * 10 FOR keeping basic observable ####
        ...
        for _, t in enumerate(timesteps):
            ...
            if noise_pred.shape[0] > 1:
                for idx in range(noise_pred.shape[0]):
                    h_t_output.append(
                        self.scheduler.step(noise_pred[idx:idx + 1], t[idx:idx + 1].long().item(),
                                            h_t[idx:idx + 1], generator=generator).prev_sample)
